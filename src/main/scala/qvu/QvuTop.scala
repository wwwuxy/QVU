 ...
 */
 ...
 class QvuTop(
   ...
   val io = IO(new Bundle {
     ...
     val int_o   = Output(Vec(MAX_VECTOR_SIZE, SInt(INT_WIDTH.W)))  // 整数输出接口
     
     // 重置滑动窗口信号
     val reset_window = Input(Bool())  // 用于重置量化模块中的滑动窗口
 })

  // 添加decode模块实例，只保留一个
  val decode = Module(new PositDecode(LIMITED_POSIT_WIDTH, LIMITED_VECTOR_SIZE, ES))

  // 添加float decode模块实例，只保留一个
  val floatDecode = Module(new FloatDecode(float_exp_width, float_frac_width, LIMITED_VECTOR_SIZE))

  // 添加Wire定义 - 使用统一的中间结果存储，移除第二组Wire
  ...
  // 初始化FloatDecode模块的输入
  floatDecode.io.float := io.float_i
  ...
  for(i <- 0 until LIMITED_VECTOR_SIZE) {
   ...
   when(io.Isposit) {
     decode.io.posit     := io.posit_i
     floatDecode.io.float := VecInit(Seq.fill(MAX_VECTOR_SIZE)(0.U(FLOAT_WIDTH.W)))
     // 只拷贝有效范围内的结果
     for (i <- 0 until MAX_VECTOR_SIZE) {
       when(valid_range(i)) {
         pir_sign(i) := decode.io.Sign(i)
         pir_exp(i)  := decode.io.Exp(i)
         pir_frac(i) := decode.io.Frac(i)
       }
     }
   }.otherwise {
     decode.io.posit     := VecInit(Seq.fill(MAX_VECTOR_SIZE)(0.U(MAX_POSIT_WIDTH.W)))
     floatDecode.io.float := io.float_i
     
     // 保存float解码结果，只处理有效元素
     for(i <- 0 until MAX_VECTOR_SIZE) {
       when(valid_range(i)) {
         float_data(i).sign   := floatDecode.io.Sign(i)
         float_data(i).exp    := floatDecode.io.Exp(i)
         float_data(i).frac   := floatDecode.io.Frac(i)
         float_data(i).isNaN  := floatDecode.io.isNaN(i)
         float_data(i).isInf  := floatDecode.io.isInf(i)
         float_data(i).isZero := floatDecode.io.isZero(i)
   ...
   when(io.op === 0.U) {  // QuantizeInt8 - Posit量化至Int8（原OP10改为OP0）
     ...
     // 添加定期重置窗口功能，每32个周期重置一次，确保量化范围正确更新
     val resetCounter = RegInit(0.U(5.W))
     resetCounter := resetCounter + 1.U
     quantizeInt8.io.reset_window := resetCounter === 0.U || io.reset_window
     
     // 调试模式下输出内部状态
     when(resetCounter === 0.U || io.reset_window) {
       printf("Quant stats: max=%d, min=%d, scale=%d, offset=%d\n", 
              quantizeInt8.global_max, 
              quantizeInt8.global_min,
              quantizeInt8.global_scale,
              quantizeInt8.global_offset)
     }
   ...
   .elsewhen(io.op === 1.U) {  // QuantizeInt4 - Posit量化至Int4
     val quantizeInt4 = Module(new PositQuantizeToInt4(
       MAX_POSIT_WIDTH,
       MAX_VECTOR_SIZE,
       MAX_ALIGN_WIDTH,
       ES
     ...
     // 输入PIR格式的posit数据
     quantizeInt4.io.pir_sign_i := pir_sign
     quantizeInt4.io.pir_exp_i  := pir_exp
     quantizeInt4.io.pir_frac_i := pir_frac
     quantizeInt4.io.reset_window := false.B  // 默认不重置窗口
