...
struct TestData {
    ...
    uint32_t golden[SAMPLE_NUM][4];     // 预期结果仍为posit32格式     
};
...
int main(int argc, char** argv) {
            ...
            if (!posit_equal(hw_result[j], golden[j])) {
                ...
                if (error < 2.0) {
                    ...
                    std::cout << "轻微差异: 样本 " << i << " 元素 " << j << "\n"
                              ...
                              << "  输入数据: 被除数=" << std::fixed << act[j] 
                              << ", 除数=" << std::fixed << weight[j] << "\n"
                              << "  相对误差: " << std::dec << error << "%\n";
                    */
                } else if (error < 5.0) {
                    // 误差在2%-5%之间，视为轻微错误
    ...
    // 结果报告
    std::cout << "\n验证结果\n========="
              << "\nfp32到posit32的div测试完成！"
              << "\n总样本数: " << total
              << "\n严重错误（误差≥5%）: " << severe_errors << " (" << std::fixed << std::setprecision(2) << (severe_errors*100.0/total) << "%)"
              << "\n轻微错误（2%≤误差<5%）: " << minor_errors << " (" << std::fixed << std::setprecision(2) << (minor_errors*100.0/total) << "%)"
              << "\n轻微差异（误差<2%）: " << minor_diff_count << " (" << std::fixed << std::setprecision(2) << (minor_diff_count*100.0/total) << "%)"
              << "\n平均误差: " << std::fixed << std::setprecision(2) << avg_error << "%\n";

    // 定义测试通过条件：严重错误不超过0.5%
    bool test_pass = (severe_errors * 100.0 / total) < 0.5;
    std::cout << "测试结果: " << (test_pass ? "通过" : "失败") << std::endl;
