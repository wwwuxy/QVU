...
/**
 * PositLess模块 - 比较两个解码后的posit向量，选择较小值并直接输出编码后的Posit
 * 
...
class PositLess(
  ...
  for (i <- 0 until MAX_VECTOR_SIZE) {
    ...
    val is_less = WireDefault(true.B)
    ...
    when(is_nar1 && is_nar2) {
      ...
      when(is_zero1 && is_zero2) {
        ...
        // 只有第一个是0，只有当第二个输入为负数时，第一个输入（0）更大
        is_less := io.pir_sign2_i(i) === 0.U
      }.elsewhen(is_zero2) {
        // 只有第二个是0，只有当第一个输入为负数时，第二个输入（0）更大
        is_less := io.pir_sign1_i(i) === 1.U
